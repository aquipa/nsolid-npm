...
async function checkRuntime (lts) {
  const { tarUrl, version } = await checkVersion(lts)
  // @TODO also check the binary existence and if it works
  if (!fs.existsSync(runtimePath)) await downloadRuntime(tarUrl, version)
}
...
async function checkVersion (lts) {
  ...
  if (platform === 'linux' && arch === 'arm64') artifactVersion = 'linuxArm64BundleUrl'
  ...
  try {
    ...
    for (const key in availableVersions) {
      if (availableVersions[key]) {
        version = key
        tarUrl = availableVersions[key][artifactVersion]
        break
      }
  ...
  return { tarUrl, version }
}

// Download a specified runtime
async function downloadRuntime (tarUrl, version) {
  ...
  try {
    ...
    execSync(extractCommand(method, path.join(runtimePath, '/nsolid-v'+version+'-'+process.release.lts.toLowerCase()+'-linux-arm64.tar.gz'), runtimePath))
    execSync(moveCommand(platform, path.join(runtimePath, '/nsolid-v'+version+'-'+process.release.lts.toLowerCase()+'-linux-arm64'), runtimePath))
  } catch (error) {
